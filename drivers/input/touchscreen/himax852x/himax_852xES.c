...
#include "himax_fw_truly.h"
#include "himax_fw_yushun.h"
#endif
...
#define FW_CFG_VER_FLASH_ADDR	132 //FW version addr offset
...
static int himax_input_register(struct himax_ts_data *ts)
{
	...
	pr_debug("abs_x_min=%d, abs_x_max=%d, abs_y_min=%d, abs_y_max=%d\n",
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR,
		ts->pdata->abs_pressure_min, ts->pdata->abs_pressure_max, ts->pdata->abs_pressure_fuzz, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE,
		ts->pdata->abs_pressure_min, ts->pdata->abs_pressure_max, ts->pdata->abs_pressure_fuzz, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_WIDTH_MAJOR,
		ts->pdata->abs_width_min, ts->pdata->abs_width_max, ts->pdata->abs_pressure_fuzz, 0);
...
static void calc_data_size(uint8_t finger_num)
{
	...
	pr_debug("coord_data_size=%d, area_data_size=%d, raw_data_frame_size=%d, raw_data_nframes=%d",
...
static void himax_touch_information(void)
{
	...
	if(data[2]==0x02)//2T2R type panel
		Is_2T2R = true;
	else
		Is_2T2R = false;
...
{
	unsigned char* ImageBuffer = i_CTPM_FW_TRULY;
	int fullFileLength = sizeof(i_CTPM_FW_TRULY);
	uint8_t checksumResult = 0;
	char data[12]= {0};
	...
	if (private_ts->vendor_sensor_id == 0x11) {
		...
		ImageBuffer= i_CTPM_FW_YUSHUN;
		fullFileLength= sizeof(i_CTPM_FW_YUSHUN);
		pr_info("ctp module_name = yushun\n");
	} else {
	...
	pr_info("current fw: 0x%x, new fw: 0x%x, checksum_result: 0x%x\n",
	...
	if ((private_ts->vendor_config_ver < *(ImageBuffer+ FW_CFG_VER_FLASH_ADDR)) ||
		...
		if(fts_ctpm_fw_upgrade_with_sys_fs(ImageBuffer,fullFileLength,true) == 0)
			pr_err("upgrade fw failed !\n");
		else
			pr_warn("upgrade fw success\n");
		...
		pr_info("current fw: 0x%x, vendor_fw_ver: 0x%x\n",
			(u32)private_ts->vendor_config_ver, (u32)private_ts->vendor_fw_ver);
	}
}
#endif
...
static void himax_read_tp_info(struct i2c_client *client)
{
	...
	pr_info("sensor_id=0x%x, vendor_fw_ver=0x%x\n",
		(u32)private_ts->vendor_sensor_id, (u32)private_ts->vendor_fw_ver);
}
...
#ifdef HX_SMART_WAKEUP
	if (atomic_read(&private_ts->suspend_mode) && private_ts->SMWP_enable) {
#else
	if (atomic_read(&private_ts->suspend_mode)) {
#endif
		buf[0] = 0x8F;
		buf[1] = 0x20;
		ret = i2c_himax_master_write(private_ts->client, buf, 2, HIMAX_I2C_RETRY_TIMES);
		if (ret < 0)
			pr_err("i2c write reg 0x8f failed %d\n", ret);
		pr_info("into wakeup mode\n");
	}
	pr_info("esd reset done.\n");
}
#endif
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	...
	if (i2c_himax_read(ts->client, 0x86, buf, 32, HIMAX_I2C_RETRY_TIMES))
		pr_err("can't read data from chip !\n");

	if ((buf[0]==0x80) && (buf[1]==0x80) &&
		(buf[2]==0x80) && (buf[3]==0x80) &&
		(buf[4]==0x80) && (buf[5]==0x80) && (buf[6]==0x80)) {
		return 1;
	} else {
		I("%s: not wake packet, buf: %x, %x, %x, %x\n",
			__func__, (u32)buf[0], (u32)buf[1], (u32)buf[2], (u32)buf[3]);
		return 0;
	}
}
#endif
...
static void himax_ts_work(struct himax_ts_data *ts)
{
	...
	if (HX_ON_HAND_SHAKING) {//chip on hand shaking,wait hand shaking
		for (j = 0; j < 100; j++) {
			if (HX_ON_HAND_SHAKING == 0) {//chip on hand shaking end
				pr_debug("HX_ON_HAND_SHAKING OK check %d times\n", j);
				break;
			} else {
				msleep(1);
			}
		...
		if (j == 100) {
			pr_err("HX_ON_HAND_SHAKING timeout reject interrupt\n");
			return;
		}
	}
#endif
	...
	if (i2c_himax_read(ts->client, 0x86, buf, hx_touch_info_size,HIMAX_I2C_RETRY_TIMES)) {
...
#endif
		{
			...
			pr_err("Back from reset, ready to serve\n");
			return;
		...
		if (HW_RESET_ACTIVATE) {
			...
			I("HW_RST Back from reset, ready to serve\n");
			return;
		}
...
static irqreturn_t himax_ts_thread(int irq, void *ptr)
{
	...
	pr_info("irq start time = %ld.%06ld s\n",
...
static ssize_t himax_debug_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
...
firmware_upgrade_err:
	...
	HX_CHIP_POLLING_COUNT = 0;
	queue_delayed_work(private_ts->himax_chip_monitor_wq,
		&private_ts->himax_chip_monitor, HX_POLLING_TIMES*HZ);
#endif
	return count;
}
	...
	struct himax_i2c_platform_data *pdata)
{
	...
	pr_debug("display-coords = (%d, %d)", pdata->screenWidth, pdata->screenHeight);
	...
	if (!gpio_is_valid(pdata->gpio_3v3_en)) {
		pr_warn("gpio_3v3_en value is not valid\n");
	}
	pr_debug("gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d\n",
...
#ifdef HX_SMART_WAKEUP
static ssize_t himax_gesture_proc_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	int cnt= 0;
	char *page = NULL;
...
static ssize_t himax_gesture_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	char i;
	struct himax_ts_data *ts = private_ts;
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(is_tp_driver_loaded) {
		pr_err("touch driver has been loaded, return !\n");
		return -ENODEV;
	}
	...
	if (!ts->himax_chip_monitor_wq) {
		pr_err("create himax_chip_monitor_wq failed !\n");
		err = -EFAULT;
...
//	enable_irq_wake(client->irq);
	...
	memset(tp_version, 0, sizeof(tp_version));
	sprintf(tp_version, "[fw]0x%2.2X,[ic]HX8527", private_ts->vendor_config_ver);
	if (0x11 == private_ts->vendor_sensor_id)
...
static int himax852xes_suspend(struct device *dev)
{
	...
	if(atomic_read(&ts->suspend_mode)) {
		pr_info("already suspended, skipped\n");
		return 0;
	}
	...
	if (HX_ON_HAND_SHAKING) {//chip on hand shaking,wait hand shaking
		...
		if (t == 100) {
			pr_err("HX_ON_HAND_SHAKING timeout reject suspend\n");
			return 0;
		}
	}
#endif
...
static int himax852xes_resume(struct device *dev)
{
	...
	if (!atomic_read(&ts->suspend_mode)) {
		pr_info("already resumed, skipped\n");
		return 0;
	}
...
#ifdef HX_CHIP_STATUS_MONITOR
	/* chip on hand shaking,wait hand shaking */
	if (HX_ON_HAND_SHAKING) {
		...
		if (t == 100) {
			pr_err("HX_ON_HAND_SHAKING timeout reject resume\n");
			return 0;
		}
	}
#endif
